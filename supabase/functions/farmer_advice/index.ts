...
serve(async (req) => {
  ...
  try {
    ...
    const { data: crop, error: cropError } = await supabase
      .from("crops")
      .select("id, crop_name, status, mvp_status, expected_harvest_date, harvest_estimate, expected_quantity_kg, estimated_quantity, district")
      .eq("id", crop_id)
...
Crop: ${crop.crop_name}
Status: ${crop.mvp_status || crop.status}
Expected harvest date: ${crop.expected_harvest_date || crop.harvest_estimate || "unknown"}
Expected quantity (kg): ${crop.expected_quantity_kg ?? crop.estimated_quantity ?? "unknown"}
